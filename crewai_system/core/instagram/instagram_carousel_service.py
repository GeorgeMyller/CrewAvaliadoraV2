...
class InstagramCarouselService(BaseInstagramService):
    ...
    def _validate_token(self, force_check=False):
        ...
        try:
            ...
            if response and "data" in response and response["data"].get("is_valid"):
                ...
                if "instagram_content_publishing" not in scopes:
                    logger.warning(
                        "Token is missing 'instagram_content_publishing' permission - REQUIRED for posting!"
                    )

                # Check for other important permissions
                missing_perms = []
                required_perms = ["instagram_basic", "instagram_content_publishing"]
                ...
                if self.token_expires_at:
                    logger.info(
                        f"Token will expire at: {datetime.fromtimestamp(self.token_expires_at)}"
                    )
    ...
    def _refresh_token(self):
        ...
        try:
            ...
            logger.info(
                f"Token refreshed. New expiration: {datetime.fromtimestamp(self.token_expires_at)}"
            )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code != 200:
                    logger.error(
                        f"Media URL not accessible: {media_url}, status code: {response.status_code}"
                    )
                    ...
                    if response.status_code == 429:
                        ...
                        logger.warning(
                            f"Rate limit hit from image host. Waiting {retry_after}s before retry..."
                        )
                ...
                if "too many 429 error responses" in str(e) or "429" in str(e):
                    ...
                    logger.warning(
                        f"Rate limit hit from image host. Waiting {delay:.2f}s before retry..."
                    )
                    time.sleep(delay)
                elif attempt < max_retries - 1:
    ...
    def _create_child_container(self, media_url: str) -> Optional[str]:
        ...
        params = {"image_url": media_url, "is_carousel_item": "true"}
    ...
    def create_carousel_container(
        ...
        if len(valid_media_urls) < 2:
            logger.error(
                f"Not enough valid media URLs to create carousel. Found: {len(valid_media_urls)}, required: at least 2"
            )
            return None
        ...
        if len(children) < 2:
            logger.error(
                f"Not enough child containers created. Found: {len(children)}, required: at least 2"
            )
            return None
    ...
    def wait_for_container_status(
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if not data:
                    logger.warning(
                        f"Failed to get container status (attempt {attempt + 1}/{max_attempts})"
                    )
                    time.sleep(delay)
                    continue
                ...
                if status_code == "FINISHED" and not publishing_to_ig:
                    ...
                    logger.error("Container failed with error:")
                ...
                # Best-effort handling without assuming 'response' attribute on custom error types
    ...
    def post_carousel(self, media_urls: List[str], caption: str) -> Optional[str]:
        ...
        if len(media_urls) < 2 or len(media_urls) > 10:
            raise ValueError(
                f"Invalid number of media URLs. Found: {len(media_urls)}, required: 2-10"
            )
    ...
    def check_token_permissions(self):
        ...
        try:
            ...
            required_permissions = ["instagram_basic", "instagram_content_publishing"]
