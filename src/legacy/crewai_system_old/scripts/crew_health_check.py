...
def check_python_version():
    ...
    print_status(
        ...
        is_valid,
        f"Requerido: Python {required_major}.{required_minor}+"
        if not is_valid
        else "Versão adequada",
    )
...
def check_file_exists(file_path):
    ...
    print_status(f"Arquivo: {file_path}", exists, "Encontrado" if exists else "Não encontrado")
    return exists
...
"""
    ...
    with open(
        f"health_check_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md", "w", encoding="utf-8"
    ) as f:
        f.write(report)
...
def main():
    ...
    results["UV Package Manager"] = check_file_exists("uv.lock") or check_file_exists(
        "pyproject.toml"
    )
