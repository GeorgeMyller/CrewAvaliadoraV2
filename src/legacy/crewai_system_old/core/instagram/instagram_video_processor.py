...
class VideoProcessor:
    ...
    def check_codec(video_codec, audio_codec):
        ...
        return video_codec.startswith("libx264") and (
            audio_codec is None or audio_codec.startswith("aac")
        )  # O audio pode não existir
        ...
        try:
            with VideoFileClip(video_path) as clip:
                ...
                if not VideoProcessor.check_duration(video_info["duration"], post_type):
                    ...
                    clip = clip.subclip(
                        0, min(video_info["duration"], max_duration)
                    )  # Corta o video, caso necessário.

                if not VideoProcessor.check_resolution(
                    video_info["width"], video_info["height"], post_type
                ):
    ...
    def validate_video(video_path, post_type="reels"):
        ...
        try:
            ...
            if info["duration"] < min_duration:
                issues.append(
                    f"Vídeo muito curto (duração: {info['duration']:.1f}s, mínimo: {min_duration}s)"
                )

            if info["duration"] > max_duration:
                issues.append(
                    f"Vídeo muito longo (duração: {info['duration']:.1f}s, máximo: {max_duration}s)"
                )
            ...
            if info["width"] < min_resolution or info["height"] < min_resolution:
                issues.append(
                    f"Resolução muito baixa ({info['width']}x{info['height']}, mínimo recomendado: {min_resolution}px)"
                )
            ...
            if post_type == "reels" or post_type == "carousel":
                ...
                if aspect_ratio > 0.8:  # Muito largo
                    issues.append(
                        f"Proporção inadequada para {post_type} ({aspect_ratio:.2f}:1, ideal 9:16 = 0.56:1)"
                    )
            ...
            if file_size_mb > max_file_size_mb:
                issues.append(
                    f"Tamanho do arquivo excede o limite ({file_size_mb:.1f}MB, máximo: {max_file_size_mb}MB)"
                )
    ...
    @staticmethod
    def force_optimize_for_instagram(
        video_path: str, output_path: str = None, post_type: str = "reels"
    ) -> str:
        """
        ...
        try:
            ...
            if output_path is None:
                ...
                output_path = os.path.join(
                    tempfile.gettempdir(), f"{name}_optimized_{post_type}.mp4"
                )
            ...
            cmd = [
                ...
                "fast",
                "-profile:v",
                "baseline",  # Melhor compatibilidade
                "-pix_fmt",
                "yuv420p",  # Formato de pixel recomendado
                "-b:v",
                "4000k",  # Bitrate de vídeo
                "-maxrate",
                "4000k",  # Bitrate máximo
                "-bufsize",
                "8000k",  # Tamanho do buffer
                "-c:a",
                "aac",  # Codec de áudio
                "-b:a",
            ...
            if result.returncode != 0:
                logger.error(
                    f"Falha na otimização forçada: {result.stderr.decode('utf-8', errors='replace')}"
                )
                return None
    ...
    def _optimize_video(self, video: mp.VideoFileClip, specs: dict) -> mp.VideoFileClip | None:
        ...
        try:
            ...
            if video.duration > specs["max_duration"]:
                logger.info(
                    f"Video duration ({video.duration}s) exceeds maximum ({specs['max_duration']}s). Trimming..."
                )
                video = video.subclip(0, specs["max_duration"])
            elif video.duration < specs["min_duration"]:
                logger.error(
                    f"Video too short ({video.duration}s). Minimum duration is {specs['min_duration']}s"
                )
                return None
