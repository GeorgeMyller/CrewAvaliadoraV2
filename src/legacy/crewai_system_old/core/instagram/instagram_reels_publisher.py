...
- Validação de vídeos conforme requisitos de Reels (duração, formato, dimensões, tamanho, proporção).
- Upload de vídeos e thumbnails locais para Imgur, obtendo URLs públicas.
- Criação de containers de mídia para Reels via API do Instagram.
- Monitoramento do status do processamento do vídeo com tratamento de erros e limites de requisições.
- Publicação do Reels no perfil do usuário, com opções de legenda, hashtags, áudio personalizado e thumbnail.
- Formatação automática de legendas com hashtags.
- Classe auxiliar `ReelsValidator` para validação detalhada dos arquivos de vídeo antes da publicação.

Exceções e erros são tratados e registrados via logging, facilitando o diagnóstico de problemas durante o processo de publicação.
Módulo especializado para publicação de Reels no Instagram
...
class ReelsPublisher(BaseInstagramService):
    ...
    def create_reels_container(
        ...
        # v23 usa cover_url e suporta thumb_offset
        if cover_url:
    ...
    def post_reels(
        ...
        result = {"id": post_id, "container_id": container_id, "media_type": "REELS"}
    ...
    def upload_local_video_to_reels(
        self,
        video_path,
        caption,
        hashtags=None,
        ...
        try:
            ...
            # Upload cover image if provided (formerly thumbnail)
            if thumbnail_path and os.path.exists(thumbnail_path):
                ...
                if thumb_result and "link" in thumb_result:
                    cover_url = thumb_result["link"]
                    logger.info(f"Cover enviada: {cover_url}")

            logger.info("Enviando vídeo para Imgur...")
...
class ReelsValidator:
    ...
    def validate(cls, video_path):
        ...
        if file_size_mb > cls.MAX_SIZE_MB:
            return (
                False,
                f"Video size exceeds {cls.MAX_SIZE_MB}MB (actual: {file_size_mb:.2f}MB)",
            )
        ...
        if ext.lower().replace(".", "") not in cls.ALLOWED_FORMATS:
            return (
                False,
                f"Video format not supported. Use: {', '.join(cls.ALLOWED_FORMATS)}",
            )
        ...
        try:
            with VideoFileClip(video_path) as clip:
                ...
                if duration < cls.MIN_DURATION:
                    return (
                        False,
                        f"Video too short ({duration:.1f}s). Minimum duration is {cls.MIN_DURATION}s",
                    )
                if duration > cls.MAX_DURATION:
                    return (
                        False,
                        f"Video too long ({duration:.1f}s). Maximum duration is {cls.MAX_DURATION}s",
                    )
                ...
                if width < cls.MIN_WIDTH:
                    return (
                        False,
                        f"Video width too small ({width}px). Minimum width is {cls.MIN_WIDTH}px",
                    )
                if height < cls.MIN_HEIGHT:
                    return (
                        False,
                        f"Video height too small ({height}px). Recommended minimum height is {cls.MIN_HEIGHT}px",
                    )
                ...
                if aspect_ratio > 1.91 or aspect_ratio < 0.5:
                    return (
                        False,
                        f"Video aspect ratio ({aspect_ratio:.2f}) outside of recommended range (0.5-1.91)",
                    )
