"""
Módulo responsável pela publicação de fotos no Instagram.

Classes:
    PostPublisher: Especializada em publicar fotos no Instagram, gerenciando o fluxo completo desde o upload da imagem até a publicação do post.

Atributos:
    POST_CONFIG (dict): Configurações de requisitos para publicação de fotos (proporção, resolução, tamanho, formatos suportados).

Métodos:
    __init__(access_token=None, ig_user_id=None):
        Inicializa o serviço com credenciais do Instagram.

    create_container(image_url, caption):
        Cria um container de mídia para a foto a ser publicada.

    publish_photo(container_id):
        Publica a foto no Instagram utilizando o container criado.

    upload_photo(photo_path, caption):
        Realiza o fluxo completo de postagem de uma foto: upload, criação de container, processamento e publicação.

    _upload_to_cdn(photo_path):
        Realiza o upload da foto para um CDN ou armazenamento temporário.

Notas:
    - As credenciais do Instagram devem ser definidas nas variáveis de ambiente.
    - O upload para CDN utiliza o serviço do Imgur como solução temporária.
    - O módulo utiliza logging para registrar operações e erros.
...
class PostPublisher(BaseInstagramService):
    ...
    def upload_photo(self, photo_path, caption):
        ...
        try:
            ...
            return {"id": post_id, "permalink": permalink, "media_type": "IMAGE"}
