...
class CarouselNormalizer:
    ...
    def validate_for_instagram(image_path: str) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if info["width"] < CarouselNormalizer.MIN_WIDTH:
                issues.append(
                    f"Width too small: {info['width']}px (min: {CarouselNormalizer.MIN_WIDTH}px)"
                )
            if info["height"] < CarouselNormalizer.MIN_HEIGHT:
                issues.append(
                    f"Height too small: {info['height']}px (min: {CarouselNormalizer.MIN_HEIGHT}px)"
                )
            if info["width"] > CarouselNormalizer.MAX_WIDTH:
                issues.append(
                    f"Width too large: {info['width']}px (max: {CarouselNormalizer.MAX_WIDTH}px)"
                )
            if info["height"] > CarouselNormalizer.MAX_HEIGHT:
                issues.append(
                    f"Height too large: {info['height']}px (max: {CarouselNormalizer.MAX_HEIGHT}px)"
                )

            # Check aspect ratio
            if info["aspect_ratio"] < CarouselNormalizer.MIN_ASPECT_RATIO:
                issues.append(
                    f"Aspect ratio too narrow: {info['aspect_ratio']} (min: {CarouselNormalizer.MIN_ASPECT_RATIO})"
                )
            if info["aspect_ratio"] > CarouselNormalizer.MAX_ASPECT_RATIO:
                issues.append(
                    f"Aspect ratio too wide: {info['aspect_ratio']} (max: {CarouselNormalizer.MAX_ASPECT_RATIO})"
                )
    ...
    def get_most_common_ratio(image_paths: List[str]) -> float:
        ...
        most_common = max(ratio_counts, key=ratio_counts.__getitem__)
    ...
    def get_best_instagram_ratio(current_ratio: float) -> float:
        ...
        if (
            CarouselNormalizer.MIN_ASPECT_RATIO
            <= current_ratio
            <= CarouselNormalizer.MAX_ASPECT_RATIO
        ):
        ...
        return float(rec_ratios[closest])
    ...
    def normalize_image(image_path: str, target_ratio: float) -> Optional[str]:
        ...
        try:
            ...
            if target_ratio < CarouselNormalizer.MIN_ASPECT_RATIO:
                ...
                logger.warning(
                    f"Target ratio {target_ratio} is below Instagram's minimum. Adjusting to {adjusted_target_ratio}"
                )
            elif target_ratio > CarouselNormalizer.MAX_ASPECT_RATIO:
                adjusted_target_ratio = CarouselNormalizer.MAX_ASPECT_RATIO
                logger.warning(
                    f"Target ratio {target_ratio} exceeds Instagram's maximum. Adjusting to {adjusted_target_ratio}"
                )
            ...
            with Image.open(image_path) as img:
                ...
                if abs(orig_ratio - adjusted_target_ratio) < 0.01:
                    ...
                    logger.info(
                        f"Image {image_path} already has suitable ratio ({orig_ratio:.3f}), saved with possible resizing"
                    )
                    return temp_path
                ...
                logger.info(
                    f"Normalized image {image_path} from {orig_width}x{orig_height} (ratio: {orig_ratio:.3f}) "
    ...
    def normalize_carousel_images(image_paths: List[str]) -> List[str]:
        ...
        for path in valid_paths:
            ...
            if info:
                logger.info(
                    f"- {os.path.basename(path)}: {info['width']}x{info['height']}, "
