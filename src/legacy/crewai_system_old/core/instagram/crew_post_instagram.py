"""
Módulo para geração automatizada de postagens para Instagram utilizando CrewAI. (Referência: Professor Sandeco - Livro CrewAI2 - https://physia.com.br/crewai2/)
Este módulo define a classe InstagramPostCrew, responsável por criar legendas envolventes e criativas para postagens no Instagram, integrando insumos fornecidos pelo usuário e destacando os benefícios da IA, especialmente da AcessoIA, para produtividade, inclusão digital e acesso ao mercado de trabalho.
Principais funcionalidades:
- Configuração de agentes e tarefas utilizando CrewAI e LLM Gemini.
- Geração de legendas com tom irreverente, profissional e humorístico, adaptando o texto para referência à AcessoIA.
- Incorporação de zoeiras numeradas, hashtags relevantes e transformação de linguagem pessoal para comunicação empresarial.
- Suporte a diferentes formatos de entrada (dicionário ou XML), com fallback para valores padrão.
- Retorno de uma postagem formatada para Instagram, incluindo legenda e hashtags.
Classes:
    InstagramPostCrew:
        Classe principal para orquestrar a geração de postagens no Instagram via CrewAI.
Métodos:
    __init__():
        Inicializa os serviços, configura o LLM Gemini e prepara os agentes e tarefas.
    create_crew():
        Configura agentes e tarefas para geração de legendas e hashtags conforme insumos recebidos.
    kickoff(inputs):
        Executa o processo de geração da postagem, recebendo insumos do usuário e retornando a legenda formatada e hashtags.
Uso:
    Instancie a classe InstagramPostCrew e utilize o método kickoff passando os insumos desejados para obter uma postagem pronta para o Instagram.
...
class InstagramPostCrew:
    ...
    def create_crew(self):
        ...
        captioner = Agent(
            ...
            goal="""
Escrever legendas divertidas, sempre envolventes 
        ...
        if not isinstance(inputs, dict):
            ...
            if isinstance(inputs, str) and "<genero>" in inputs:
                try:
                    ...
                    logger.error(f"Erro ao converter entrada XML para dicionário: {str(e)}")
                    # Use defaults
                    inputs = {
                        "genero": "Neutro",
                ...
                logger.warning("Formato de entrada não reconhecido. Usando valores padrão.")
                inputs = {
                    "genero": "Neutro",
                    "caption": "Imagem para Instagram",
                    "describe": "Imagem para redes sociais",
