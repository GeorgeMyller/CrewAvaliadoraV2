...
- Obtenção de informações do vídeo (dimensões, duração, tamanho, formato, proporção).
- Validação dos requisitos do Instagram Reels (duração, formato, tamanho do arquivo).
- Processamento do vídeo para ajustar dimensões e proporção recomendadas.
- Limpeza de arquivos temporários antigos gerados durante o processamento.

Constantes de requisitos do Instagram Reels:
- Duração mínima: 3 segundos
- Duração máxima: 90 segundos
- Proporção recomendada: 9:16 (vertical)
- Dimensões recomendadas: 1080x1920 pixels
- Tamanho máximo do arquivo: 100 MB
- Formatos suportados: mp4, mov

Exceções são registradas via logging para facilitar o diagnóstico de problemas.
...
class VideoProcessor:
    ...
    def validate_video(cls, video_path: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if not format_valid:
                return (
                    False,
                    f"Formato de vídeo não suportado. Use: {', '.join(cls.SUPPORTED_FORMATS)}",
                )
            ...
            if duration < cls.MIN_DURATION_SECONDS:
                return (
                    False,
                    f"Duração do vídeo muito curta: {duration:.1f}s. Mínimo: {cls.MIN_DURATION_SECONDS}s",
                )

            if duration > cls.MAX_DURATION_SECONDS:
                return (
                    False,
                    f"Duração do vídeo muito longa: {duration:.1f}s. Máximo: {cls.MAX_DURATION_SECONDS}s",
                )
            ...
            if video_info["file_size_mb"] > cls.MAX_FILE_SIZE_MB:
                return (
                    False,
                    f"Tamanho do arquivo muito grande: {video_info['file_size_mb']:.1f}MB. Máximo: {cls.MAX_FILE_SIZE_MB}MB",
                )
    ...
    def process_video_for_reels(cls, input_path: str, output_path: str | None = None) -> str:
        ...
        try:
            ...
            if (
                video_info["width"] == cls.RECOMMENDED_WIDTH
                and video_info["height"] == cls.RECOMMENDED_HEIGHT
                and any(fmt in video_info["format"] for fmt in cls.SUPPORTED_FORMATS)
            ):
