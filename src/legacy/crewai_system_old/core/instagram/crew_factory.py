...
class CrewFactory:
    ...
    def create_crew_for_group(self, group_id: str) -> AdvancedInstagramPostCrew:
        ...
        if not group_config:
            logger.warning(
                f"⚠️  Configuração para grupo {group_id} não encontrada. Usando configuração padrão."
            )
        ...
        logger.info(
            f"✅ Criando CrewAI personalizado para grupo: {group_config.get('group_name', group_id)}"
        )
