...
def clone_github_repo(url: str, target_dir: Optional[str] = None, timeout: int = GIT_CLONE_TIMEOUT_SECONDS) -> str:
    ...
    try:
        ...
        logger.info(f"✅ Repositório clonado com sucesso em: {target_dir}")
        return target_dir
        ...
        error_msg = f"❌ Erro ao clonar repositório: {e.stderr}"
        logger.error(error_msg)
        # Cleanup em caso de erro
        if os.path.exists(target_dir):
            shutil.rmtree(target_dir, ignore_errors=True)
        ...
        error_msg = f"❌ Erro inesperado ao clonar repositório: {str(e)}"
        logger.error(error_msg)
        # Cleanup em caso de erro
        if os.path.exists(target_dir):
            shutil.rmtree(target_dir, ignore_errors=True)
...
class CodebaseAnalysisCrew:
    ...
    def  run_analysis(self, report_path: str = "relatorio_codebase_turbinado.md",
        ...
        Comportamento:
        - Se `report_path` é uma URL do GitHub: clona o repositório e analisa
        ...
        # Variável para rastrear diretório clonado que precisa de cleanup
        cloned_dir = None
        
        try:
            # Verifica se é uma URL do GitHub e clona se necessário
            if is_github_url(report_path):
                logger.info(f"🔗 URL do GitHub detectada: {report_path}")
                cloned_dir = clone_github_repo(report_path)
                # Atualiza report_path para o diretório clonado
                report_path = cloned_dir
                logger.info(f"✅ Repositório clonado. Analisando: {report_path}")
    ...
    """,
            ...
            # Cleanup: remove diretório clonado se necessário
            if cloned_dir and os.path.exists(cloned_dir):
                try:
                    logger.info(f"🧹 Limpando diretório temporário: {cloned_dir}")
                    shutil.rmtree(cloned_dir, ignore_errors=True)
                    logger.info("✅ Limpeza concluída")
                except Exception as cleanup_error:
                    logger.warning(f"⚠️ Erro ao limpar diretório temporário: {cleanup_error}")
